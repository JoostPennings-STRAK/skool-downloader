...
- `src/server.ts` + `web/index.html`: local browser UI (`npm run skool-web`, `http://localhost:4471`, auto-opens). Endpoints: `GET /api/thread?url=`, `POST /api/download` (streams NDJSON progress), `GET /api/file?path=` (serves downloaded mp4s, confined to `downloads/`), `GET /api/auth`. Renders the comment tree with checkboxes; nothing pre-checked unless `suggestedVideoIndexes` says so. No deps beyond Node `http`.
