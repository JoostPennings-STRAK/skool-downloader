...
- `src/post.ts`: Orchestrator for community **post/thread** downloads (`skool <post-url>` / `skool post <url>`). Uses `Scraper.extractPostData` to enumerate every video in a thread (post + nested comments; native Skool/Mux, or Loom/Vimeo/YouTube/Wistia embeds), resolves each to a concrete URL (HLS reconstruction, Loom `transcoded-url` API, or hand off to yt-dlp), and writes to `downloads/<Group>/Posts/<Post Title>/`. Comments are captured from the page's own `api2.skool.com/posts/{id}/comments` responses, not `__NEXT_DATA__`. `extractPostData` also returns `threadTree` (post + comments as a tree, with `isCurrentUser`/`isOwner` flags) and `suggestedVideoIndexes` (best guess at the wanted video — a `?p=` comment, or the owner's video reply under the current user's node). `downloadPost` accepts `preloaded` to skip a re-scrape.
- `src/server.ts` + `web/index.html`: browser UI (`npm run skool-web`, `http://localhost:4471`, auto-opens on darwin/win32 only). Three tabs — **Download** (thread picker), **Library** (every `.mp4` under `downloads/`), **Settings** (session import). Endpoints: `GET /api/thread?url=`, `POST /api/download` (streams NDJSON progress), `GET /api/library`, `GET /api/file?path=[&dl=1]` (serves downloaded mp4s with HTTP Range support, confined to `downloads/`; `dl=1` forces attachment), `DELETE /api/file?path=` (removes a file + prunes empty parents), `GET /api/auth`, `POST /api/auth/import` (raw `cookies.txt` body → `importCookiesTxt`). Renders the comment tree with checkboxes; nothing pre-checked unless `suggestedVideoIndexes` says so. No deps beyond Node `http`.
- `src/auth.ts` also exposes `importCookiesTxt` / `parseNetscapeCookies` / `skoolCookiesFromTxt`: a Netscape `cookies.txt` (from the "Get cookies.txt LOCALLY" extension, honouring the `#HttpOnly_` prefix) is stored raw for yt-dlp and converted to a Playwright `storage_state.json`. `src/scraper.ts` `init()` falls back to `context.addCookies()` from `cookies.txt` if the storage state carries no Skool cookies.
- `Dockerfile` + `.gitea/workflows/build-deploy.yml`: runs the web UI as a homelab service (`skool.jpmm.nl` on the MX laptop). Base image `mcr.microsoft.com/playwright:v1.58.0-noble`; `ffmpeg` via apt, `yt-dlp_linux` baked into `bin/`. Volumes: `/app/downloads`, `/app/.auth`. Stack lives in `strakmxltp-stacks/skool-downloader/`. No app-level auth. The CI runs one job on `mx-runner` that `docker build`s and then `docker compose up -d` on the same host — **no registry push** (the ~770 MB Playwright base layer trips the ingress' 413 limit), so the compose service is `pull_policy: never` and the stack is in `update_stacks.sh`'s `EXCLUDED_STACKS`.
