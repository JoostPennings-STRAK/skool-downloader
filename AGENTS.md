...
- `Dockerfile` + `.gitea/workflows/build-deploy.yml`: runs the web UI as a homelab service (`skool.jpmm.nl` on the MX laptop). Base image `mcr.microsoft.com/playwright:v1.58.0-noble`; `ffmpeg` via apt, `yt-dlp_linux` baked into `bin/`. Volumes: `/app/downloads`, `/app/.auth`. Stack lives in `strakmxltp-stacks/skool-downloader/`. No app-level auth.
